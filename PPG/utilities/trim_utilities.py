...
def trim_missing(df,as_dataframe=True):
    if as_dataframe:
...
def trim_missing_signal(df,df_origin=None,as_dataframe=True):
    if as_dataframe:
        indices_0 = np.array(df[df["PLETH"] == 0].index)
    else:
        indices_0 = np.where(df==0)[0]#np.array(df[df == 0].index)
    # indices_origin = np.array(df_origin[df_origin["PLETH"] == 0].index)
